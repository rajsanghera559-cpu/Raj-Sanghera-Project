...
def run_phase8b_no_forcing_decay_dt_half():
    ...
    # Solver logs after each update. Comparing logged step 2000 to logged step 0
    # gives 2000 dt intervals, so the physical comparison time is 5.0.
    comparison_intervals = 2000
    ...
    config = {
        "mode": "phase8B_no_forcing_single_mode_decay_dt_half",
        "Re": re_value,
        "nu": nu,
        "nx": nx,
        "ny": ny,
        ...
        "expected_diagnostic_steps": [0, 500, 1000, 1500, 2000],
        "expected_logged_energy_ratio_step2000_over_step0": expected_logged_ratio,
        "phase8A_reference": "experiments/runs/run_2026-07-03_23-57-36",
        "purpose": "timestep sensitivity check for no-forcing viscous decay benchmark using half dt",
    }

    metadata = manager.save_metadata(run_path, config, status="running")

    print("Starting Phase 8B no-forcing decay timestep sensitivity benchmark")
    print(f"Re: {re_value}")
    print(f"nu: {nu}")
    ...
    print(f"Expected logged energy ratio step2000/step0: {expected_logged_ratio:.12e}")
    ...
    print("Phase 8B no-forcing decay timestep sensitivity benchmark finished")
    print(f"Status: {status}")
    print(f"Run folder: {run_path}")


if __name__ == "__main__":
    run_phase8b_no_forcing_decay_dt_half()