...
METHOD_OUT_CSV = Path("PHASE12I_N256_THREE_METHOD_LONGER_FEASIBILITY_AUDIT.csv")
PAIRWISE_OUT_CSV = Path("PHASE12I_N256_THREE_METHOD_LONGER_FEASIBILITY_PAIRWISE.csv")
...
print("Purpose: run intermediate N256 feasibility across fd_centered, pseudo_spectral, and arakawa.")
...
reference_solver = SpectralSolver(
    ...
    ny=N,
    Re=RE,
    run_path=Path("experiments") / "reference" / "phase12I_N256_reference_grid",
    dt=DT,
    steps=STEPS,
)
...
for method in METHODS:
    solvers[method] = SelectableAdvectionSolver(
        ...
        ny=N,
        Re=RE,
        run_path=Path("experiments") / "selectable_diagnostics" / f"phase12I_N256_{method}",
        dt=DT,
        steps=STEPS,
...
dt_ok = DT == 0.001
steps_ok = STEPS == 500
final_time_ok = abs(FINAL_TIME - 0.5) <= 1e-15
log_every_ok = LOG_EVERY == 50
...
print(f"steps == 500: {pass_fail(steps_ok)}")
print(f"final time == 0.5: {pass_fail(final_time_ok)}")
print(f"log_every == 50: {pass_fail(log_every_ok)}")
...
print("\n=== OVERALL RESULT ===")
print(f"Phase 12I N256 three-method longer feasibility audit: {pass_fail(overall_pass)}")
...
print("\n=== SCIENTIFIC BOUNDARY ===")
print("This is an N256 three-method intermediate feasibility audit only.")
print("This does not prove full N256 final-time-1.0 feasibility.")
...
print("Phase 12I audit complete.")