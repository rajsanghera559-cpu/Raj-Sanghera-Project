...
RUN = Path(r"experiments\runs\run_2026-07-11_17-53-38")
METADATA_PATH = RUN / "metadata.json"
DIAG_PATH = RUN / "diagnostics.csv"
SPEC_PATH = RUN / "spectrum.csv"

REF_8A_CSV = Path("PHASE8A_NO_FORCING_DECAY_AUDIT.csv")
OUT_CSV = Path("PHASE8C_N128_DECAY_AUDIT.csv")
...
print("\n=== PHASE 8C N128 DECAY AUDIT ===")
print(f"Run folder: {RUN}")
print(f"Metadata: {METADATA_PATH}")
print(f"Diagnostics: {DIAG_PATH}")
print(f"Spectrum: {SPEC_PATH}")
print(f"Phase 8A reference CSV: {REF_8A_CSV}")

for path in [METADATA_PATH, DIAG_PATH, SPEC_PATH, REF_8A_CSV]:
    if not path.exists():
...
expected_logged_ratio_computed = float(np.exp(-2.0 * nu * k_squared * comparison_time))
...
metadata_commit_expected = str(git_commit).startswith("b7e51da")
metadata_mode_expected = mode == "phase8C_no_forcing_single_mode_decay_N128"
metadata_config_expected = (
    ...
    and comparison_intervals == 1000
    and abs(comparison_time - 5.0) < 1e-15
)
...
reference_energy_error = float(ref8a["energy_ratio_relative_error"].iloc[0])
reference_enstrophy_error = float(ref8a["enstrophy_ratio_relative_error"].iloc[0])
reference_measured_energy_ratio = float(ref8a["measured_energy_ratio"].iloc[0])
reference_measured_enstrophy_ratio = float(ref8a["measured_enstrophy_ratio"].iloc[0])

energy_ratio_matches_8A = relative_error(measured_energy_ratio, reference_measured_energy_ratio) < 1e-10
enstrophy_ratio_matches_8A = relative_error(measured_enstrophy_ratio, reference_measured_enstrophy_ratio) < 1e-10

energy_error_consistent_with_8A = abs(energy_ratio_rel_error - reference_energy_error) < 1e-9
enstrophy_error_consistent_with_8A = abs(enstrophy_ratio_rel_error - reference_enstrophy_error) < 1e-9
...
overall_pass = all(
    [
        ...
        enstrophy_energy_relation_ok,
        energy_ratio_matches_8A,
        enstrophy_ratio_matches_8A,
        energy_error_consistent_with_8A,
        enstrophy_error_consistent_with_8A,
    ]
)
...
print(f"Git commit starts with b7e51da: {pass_fail(metadata_commit_expected)}")
...
print(f"dt: {dt:.12e}")
print(f"comparison intervals: {comparison_intervals}")
print(f"comparison time: {comparison_time:.12e}")
...
print(f"Phase 8A measured energy ratio:      {reference_measured_energy_ratio:.12e}")
print(f"Phase 8C measured energy ratio:      {measured_energy_ratio:.12e}")
print(f"Energy ratio matches 8A:             {pass_fail(energy_ratio_matches_8A)}")
print(f"Phase 8A energy ratio rel error:     {reference_energy_error:.12e}")
print(f"Phase 8C energy ratio rel error:     {energy_ratio_rel_error:.12e}")
print(f"Energy error consistent with 8A:     {pass_fail(energy_error_consistent_with_8A)}")

print(f"Phase 8A measured enstrophy ratio:   {reference_measured_enstrophy_ratio:.12e}")
print(f"Phase 8C measured enstrophy ratio:   {measured_enstrophy_ratio:.12e}")
print(f"Enstrophy ratio matches 8A:          {pass_fail(enstrophy_ratio_matches_8A)}")
print(f"Phase 8A enstrophy ratio rel error:  {reference_enstrophy_error:.12e}")
print(f"Phase 8C enstrophy ratio rel error:  {enstrophy_ratio_rel_error:.12e}")
print(f"Enstrophy error consistent with 8A:  {pass_fail(enstrophy_error_consistent_with_8A)}")
...
print("\n=== OVERALL RESULT ===")
print(f"Phase 8C N128 decay audit: {pass_fail(overall_pass)}")
...
summary = {
    ...
    "phase8A_measured_energy_ratio": reference_measured_energy_ratio,
    "phase8A_energy_ratio_relative_error": reference_energy_error,
    "energy_ratio_matches_8A": pass_fail(energy_ratio_matches_8A),
    "energy_error_consistent_with_8A": pass_fail(energy_error_consistent_with_8A),
    ...
    "phase8A_measured_enstrophy_ratio": reference_measured_enstrophy_ratio,
    "phase8A_enstrophy_ratio_relative_error": reference_enstrophy_error,
    "enstrophy_ratio_matches_8A": pass_fail(enstrophy_ratio_matches_8A),
    "enstrophy_error_consistent_with_8A": pass_fail(enstrophy_error_consistent_with_8A),
...
print("Phase 8C N128 decay audit complete.")