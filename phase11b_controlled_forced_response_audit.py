...
OUT_AUDIT_CSV = Path("PHASE11B_CONTROLLED_FORCED_RESPONSE_AUDIT.csv")
OUT_TIME_CSV = Path("PHASE11B_CONTROLLED_FORCED_RESPONSE_TIME_HISTORY.csv")
OUT_PAIRWISE_CSV = Path("PHASE11B_CONTROLLED_FORCED_RESPONSE_PAIRWISE_SUMMARY.csv")
...
def audit_invalid_method():
    try:
        SelectableAdvectionSolver(
            ...
            ny=N,
            Re=RE,
            run_path=Path("experiments") / "method_diagnostics" / "phase11B_invalid",
            dt=DT,
            steps=1,
...
def make_solver(method):
    return SelectableAdvectionSolver(
        ...
        ny=N,
        Re=RE,
        run_path=Path("experiments") / "method_diagnostics" / f"phase11B_{method}",
        dt=DT,
        steps=STEPS,
...
print("Purpose: controlled forced response across fd_centered, pseudo_spectral, and arakawa.")
...
print(f"Phase 11B controlled forced-response audit: {pass_fail(overall_pass)}")

print("\n=== OUTPUT WRITTEN ===")
print(f"Wrote: {OUT_AUDIT_CSV}")
print(f"Wrote: {OUT_TIME_CSV}")
print(f"Wrote: {OUT_PAIRWISE_CSV}")
print("Phase 11B controlled forced-response audit complete.")