...
def run_phase8a_no_forcing_decay():
    ...
    # Solver logs after each update at n = 0, 500, 1000.
    # Ratio from logged step 0 to logged step 1000 uses 1000 time intervals.
    expected_logged_ratio = float(np.exp(-2.0 * nu * k_squared * (1000 * dt)))

    run_path = manager.create_run_folder()

    config = {
        "mode": "phase8A_no_forcing_single_mode_decay",
        "Re": re_value,
        "nu": nu,
        "nx": nx,
        "ny": ny,
        ...
        "purpose": "benchmark-oriented no-forcing viscous decay test for a clean single mode",
    }

    metadata = manager.save_metadata(run_path, config, status="running")

    print("Starting Phase 8A no-forcing single-mode decay benchmark")
    print(f"Re: {re_value}")
    print(f"nu: {nu}")
    ...
    print("Phase 8A no-forcing decay benchmark finished")
    print(f"Status: {status}")
    print(f"Run folder: {run_path}")


if __name__ == "__main__":
    run_phase8a_no_forcing_decay()