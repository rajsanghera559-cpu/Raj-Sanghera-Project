...
OUT_AUDIT_CSV = Path("PHASE10Y_N128_SHORT_NO_FORCING_DRIFT_COMPARISON_AUDIT.csv")
OUT_TIME_CSV = Path("PHASE10Y_N128_SHORT_NO_FORCING_DRIFT_TIME_HISTORY.csv")
OUT_PAIRWISE_CSV = Path("PHASE10Y_N128_SHORT_NO_FORCING_DRIFT_PAIRWISE_SUMMARY.csv")
...
class NoForcingSelectableAdvectionSolver(SelectableAdvectionSolver):
    """Audit-local no-forcing subclass. Source solver files remain unchanged."""
...
def audit_invalid_method():
    try:
        NoForcingSelectableAdvectionSolver(
            ...
            ny=N,
            Re=RE,
            run_path=Path("experiments") / "method_diagnostics" / "phase10Y_invalid",
            dt=DT,
            steps=1,
...
def make_solver(method):
    return NoForcingSelectableAdvectionSolver(
        ...
        ny=N,
        Re=RE,
        run_path=Path("experiments") / "method_diagnostics" / f"phase10Y_{method}",
        dt=DT,
        steps=STEPS,
...
def pairwise_final_summary(method_a, method_b, final_fields, solvers):
    ...
    no_large_pairwise_disagreement = rel_error < 0.01
...
print("Purpose: N=128 short no-forcing drift across fd_centered, pseudo_spectral, and arakawa.")
...
print(f"Phase 10Y N128 short no-forcing drift comparison audit: {pass_fail(overall_pass)}")

print("\n=== OUTPUT WRITTEN ===")
print(f"Wrote: {OUT_AUDIT_CSV}")
print(f"Wrote: {OUT_TIME_CSV}")
print(f"Wrote: {OUT_PAIRWISE_CSV}")
print("Phase 10Y N128 short no-forcing drift comparison audit complete.")