...
OUT_CSV = Path("PHASE10P1_FD_CENTERED_ONE_STEP_EQUIVALENCE_AUDIT.csv")
...
def baseline_one_step_fd_centered(solver, w):
    """
    Direct transcription of one SpectralSolver.run() RK2-style update step.

    This does not call SpectralSolver.run().
    This does not mutate solver.w.
    """
    arr = np.asarray(w)

    k1 = baseline_fd_centered_rhs(solver, arr)
    w1 = arr + solver.dt * k1

    k2 = baseline_fd_centered_rhs(solver, w1)
    w_new = arr + 0.5 * solver.dt * (k1 + k2)
...
def audit_shape_mismatch_rejection():
    solver = SelectableAdvectionSolver(
        ...
        ny=64,
        Re=1000,
        run_path=Path("experiments") / "method_diagnostics" / "phase10P1_shape_mismatch",
        dt=0.005,
        steps=1,
    ...
    try:
        solver.step_once_selectable(bad_field)
        rejected = False
    except ValueError:
        rejected = True
...
def audit_case(N, field_name, field_info):
    baseline_path = Path("experiments") / "method_diagnostics" / f"phase10P1_baseline_N{N}_{field_name}"
    selectable_path = Path("experiments") / "method_diagnostics" / f"phase10P1_selectable_N{N}_{field_name}"
    ...
    baseline_next = baseline_one_step_fd_centered(baseline_solver, w)
    selectable_next = selectable_solver.step_once_selectable(w)

    diff = selectable_next - baseline_next

    baseline_next_l2 = l2_norm(baseline_next)
    selectable_next_l2 = l2_norm(selectable_next)
    diff_l2 = l2_norm(diff)
    diff_max = max_abs(diff)
    rel_error = relative_error(diff_l2, baseline_next_l2)
    cosine = cosine_similarity(selectable_next, baseline_next)

    finite_all = (
        np.isfinite(w).all()
        and np.isfinite(baseline_next).all()
        and np.isfinite(selectable_next).all()
        and np.isfinite(diff).all()
    )

    real_all = (
        np.isrealobj(baseline_next)
        and np.isrealobj(selectable_next)
    ...
    exact_match = np.allclose(selectable_next, baseline_next, rtol=0.0, atol=0.0)
    ...
    return {
        "test_type": "fd_centered_one_step_equivalence",
        "N": N,
        ...
        "baseline_next_l2": baseline_next_l2,
        "selectable_next_l2": selectable_next_l2,
        "diff_l2": diff_l2,
...
print("\n=== PHASE 10P.1 FD_CENTERED ONE-STEP EQUIVALENCE AUDIT ===")
print("Purpose: compare direct baseline one-step logic to selectable fd_centered one-step logic.")
...
rows.append(
    {
        ...
        "baseline_next_l2": np.nan,
        "selectable_next_l2": np.nan,
        "diff_l2": np.nan,
        "diff_max_abs": np.nan,
...
for N in [64, 128]:
    ...
    field_solver = SpectralSolver(
        ...
        ny=N,
        Re=1000,
        run_path=Path("experiments") / "method_diagnostics" / f"phase10P1_field_builder_N{N}",
        dt=0.005,
        steps=1,
    )
    ...
    for field_name, field_info in fields.items():
        ...
        print(f"baseline_next_l2: {result['baseline_next_l2']:.12e}")
        print(f"selectable_next_l2: {result['selectable_next_l2']:.12e}")
        ...
        print(f"metadata_step_method_ok: {result['metadata_step_method_ok']}")
        print(f"metadata_step_status_ok: {result['metadata_step_status_ok']}")
...
print("\n=== OVERALL RESULT ===")
print(f"Phase 10P.1 fd_centered one-step equivalence audit: {pass_fail(all_pass)}")

print("\n=== OUTPUT WRITTEN ===")
print(f"Wrote: {OUT_CSV}")
print("Phase 10P.1 fd_centered one-step equivalence audit complete.")