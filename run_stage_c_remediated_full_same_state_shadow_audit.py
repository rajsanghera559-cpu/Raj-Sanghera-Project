"""
Controlled remediated full Stage C same-state advection-form shadow audit.
...
for 20,001 updates. Seven advection operators are evaluated on the same
baseline current and RK2 stage states. The pseudo-spectral shadow operators
use the verified real-compatible even-grid Nyquist convention. Historical
raw-ik calculations are confined to loop indices 3058 and 3059 for exact
failure-checkpoint reproduction. Shadow results never enter the accepted
update and never define alternate trajectories.
...
EXPECTED_LOCALIZATION_EVIDENCE_INVENTORY_SHA256 = (
    "9FF4E524A0A05ED2E2CC5214E5EE4075254D11852F8CECF895C1F22F87328D5C"
)

REMEDIATION_EVIDENCE_DIRECTORY = (
    Path("experiments")
    / "advection_form_shadow_audit_remediation"
    / "stage_c_shadow_nyquist_remediation_verification_20260720T225527Z_9a27e66"
)
EXPECTED_REMEDIATION_EVIDENCE_INVENTORY_SHA256 = (
...
EXPECTED_HISTORICAL_RAW_LOOP_INDEX = 3059
EXPECTED_HISTORICAL_RAW_STAGE = 2
EXPECTED_HISTORICAL_RAW_QUANTITY = (
    "omega_gradient_imaginary_ratio"
)
EXPECTED_HISTORICAL_RAW_RATIO = (
...
EXPECTED_CHECKPOINT_TRACE_ROWS = 40
EXPECTED_CHECKPOINT_WORK_ROWS = 6
...
RAW_CHECKPOINT_RELATIVE_TOLERANCE = 1.0e-12
RAW_CHECKPOINT_ABSOLUTE_TOLERANCE = 1.0e-18
RC_CHECKPOINT_RELATIVE_TOLERANCE = 1.0e-12
RC_CHECKPOINT_ABSOLUTE_TOLERANCE = 1.0e-18
CHECKPOINT_REAL_DERIVATIVE_RELATIVE_LIMIT = 1.0e-10
CHECKPOINT_TRANSPORT_RELATIVE_LIMIT = 1.0e-10
CHECKPOINT_WORK_ABSOLUTE_LIMIT = 1.0e-14
CHECKPOINT_WORK_RELATIVE_LIMIT = 1.0e-6
...
NYQUIST_CHECKPOINT_LOOP_INDICES = (
    3058,
    3059,
)
...
OPERATOR_REGISTRY = (
    {
        ...
        "derivative_convention": "centered_finite_difference",
    },
    {
        ...
        "primary_alternate": False,
        "derivative_convention": "centered_finite_difference_then_mask",
    },
    {
    ...
    },
    {
        ...
        "derivative_convention": (
            "real_compatible_nyquist_zeroed_spectral"
        ),
    },
    {
        "operator_id": (
        ...
        "primary_alternate": True,
        "derivative_convention": (
            "real_compatible_nyquist_zeroed_spectral_then_mask"
        ),
    },
    {
        ...
        "classification_role": "primary_alternate",
        "primary_alternate": True,
        "derivative_convention": "arakawa_periodic_jacobian",
    },
)
...
PSEUDO_RC_UNPROJECTED_ID = (
    "SHADOW_PS_ADVECTIVE_UNPROJECTED_RC_NYQUIST_V2"
)
PSEUDO_RC_PROJECTED_ID = (
    "SHADOW_PS_ADVECTIVE_PROJECTED_RC_NYQUIST_V2"
)
PSEUDO_RC_OPERATOR_IDS = (
    PSEUDO_RC_UNPROJECTED_ID,
    PSEUDO_RC_PROJECTED_ID,
)
...
STATE_REFERENCE_FIELDNAMES = (
    ...
    "stage1_maximum_imaginary_quantity",
    ...
    "stage2_maximum_imaginary_quantity",
...
NYQUIST_CHECKPOINT_TRACE_FIELDNAMES = (
    "loop_index",
    "completed_steps",
    "physical_time",
    "stage",
    ...
    "raw_to_rc_real_difference_rms",
    "raw_to_rc_real_difference_max_abs",
    "raw_to_rc_real_difference_relative",
    "raw_to_rc_real_cosine_similarity",
    "relevant_nyquist_power_fraction",
    "raw_hermitian_residual",
    "rc_hermitian_residual",
    "state_sha256",
)

NYQUIST_CHECKPOINT_WORK_FIELDNAMES = (
    "loop_index",
    "completed_steps",
    "physical_time",
    "stage",
    "operator_id",
    "historical_raw_transport_rms",
    "rc_transport_rms",
...
def verify_inventory_root_hash(
    directory: Path,
    ...
    if not directory.is_dir():
        raise RuntimeError(
            f"archived evidence directory is missing: {directory}"
        )
    ...
    if inventory_hash != expected_inventory_sha256:
        raise RuntimeError(
            f"inventory SHA256 mismatch for {inventory_path}: "
            f"observed={inventory_hash}, "
    ...
    required_files = sorted(expected_files)

    if actual_files != required_files:
        raise RuntimeError(
            f"archived evidence file set mismatch for {directory}: "
            f"observed={actual_files}, expected={required_files}"
        )

    observed: dict[str, str] = {
        "file_inventory.csv": inventory_hash,
    }
    ...
    for name in required_files:
        ...
        if name == "file_inventory.csv":
            ...
            if row is None:
                raise RuntimeError(
                    f"inventory self-entry is missing: {inventory_path}"
                )

            if str(row.get("sha256", "")).strip():
                raise RuntimeError(
                    f"inventory self-hash is not blank: {inventory_path}"
                )
...
def verify_source_identities(repo: Path) -> dict[str, str]:
    ...
    hashes["remediated_full_design"] = verify_file_hash(
        repo,
        REMEDIATED_FULL_DESIGN_PATH,
        EXPECTED_REMEDIATED_FULL_DESIGN_SHA256,
    )
    hashes["original_stage_c_design"] = verify_file_hash(
        repo,
        ORIGINAL_STAGE_C_DESIGN_PATH,
        EXPECTED_ORIGINAL_STAGE_C_DESIGN_SHA256,
    )
    hashes["original_stage_c_runner"] = verify_file_hash(
        repo,
        ORIGINAL_STAGE_C_RUNNER_PATH,
        EXPECTED_ORIGINAL_STAGE_C_RUNNER_SHA256,
    )
    hashes["localization_design"] = verify_file_hash(
        repo,
        LOCALIZATION_DESIGN_PATH,
        EXPECTED_LOCALIZATION_DESIGN_SHA256,
    )
    hashes["localization_runner"] = verify_file_hash(
        repo,
        LOCALIZATION_RUNNER_PATH,
        EXPECTED_LOCALIZATION_RUNNER_SHA256,
    )
    hashes["localization_report"] = verify_file_hash(
        repo,
        LOCALIZATION_REPORT_PATH,
        EXPECTED_LOCALIZATION_REPORT_SHA256,
    )
    hashes["remediation_design"] = verify_file_hash(
        repo,
        REMEDIATION_DESIGN_PATH,
        EXPECTED_REMEDIATION_DESIGN_SHA256,
    )
    hashes["remediation_runner"] = verify_file_hash(
        repo,
        REMEDIATION_RUNNER_PATH,
        EXPECTED_REMEDIATION_RUNNER_SHA256,
    )
    hashes["remediation_completion_report"] = verify_file_hash(
        repo,
        REMEDIATION_COMPLETION_REPORT_PATH,
        EXPECTED_REMEDIATION_COMPLETION_REPORT_SHA256,
    )
    ...
    verify_inventory(
        repo / PARTIAL_STAGE_C_DIRECTORY,
        PARTIAL_STAGE_C_HASHES,
    )

    localization_hashes = verify_inventory_root_hash(
        repo / LOCALIZATION_EVIDENCE_DIRECTORY,
        EXPECTED_LOCALIZATION_EVIDENCE_INVENTORY_SHA256,
        (
            "file_inventory.csv",
    ...
    remediation_hashes = verify_inventory_root_hash(
        repo / REMEDIATION_EVIDENCE_DIRECTORY,
        EXPECTED_REMEDIATION_EVIDENCE_INVENTORY_SHA256,
        (
            "file_inventory.csv",
    ...
    for name, value in PARTIAL_STAGE_C_HASHES.items():
        hashes[f"partial_stage_c/{name}"] = value

    for name, value in localization_hashes.items():
        hashes[f"localization/{name}"] = value

    for name, value in remediation_hashes.items():
        hashes[f"remediation/{name}"] = value
...
def build_real_compatible_wavenumbers(
    ...
    x_nyquist_mask = np.asarray(kx == nyquist_value)
    y_nyquist_mask = np.asarray(ky == nyquist_value)

    if not x_nyquist_mask.any() or not y_nyquist_mask.any():
        raise IntegrityFailure(
            "nyquist_location",
            "solver wavenumber arrays do not contain -N/2",
        )
...
def complex_measurement(
    ...
    imaginary_rms = component_values[imaginary_direction][
        "imaginary_rms"
    ]
...
def hermitian_residual(
    ...
    difference = np.sqrt(
        np.mean(np.abs(array - partner) ** 2)
    )
    scale = max(
        float(np.sqrt(np.mean(np.abs(array) ** 2))),
        RESIDUAL_FLOOR,
    )
...
def spectral_power_fraction(
    ...
    return selected / max(total, SPECTRAL_POWER_FLOOR)
...
def compute_shared_state_fields(
    ...
    *,
    ...
    helper_check: bool,
    kx_shadow: np.ndarray,
    ky_shadow: np.ndarray,
) -> dict[str, object]:
    ...
    omega_x_s, omega_y_s, omega_gradient_imaginary = (
        spectral_gradients_with_imaginary(
            state_array,
            np.asarray(kx_shadow),
            np.asarray(ky_shadow),
        )
    )
    ...
    transports = {
        ...
        "SHADOW_PS_ADVECTIVE_UNPROJECTED_RC_NYQUIST_V2": np.asarray(
            pseudo_raw_transport,
            dtype=np.float64,
        ),
        "SHADOW_PS_ADVECTIVE_PROJECTED_RC_NYQUIST_V2": np.asarray(
    ...
    maximum_imaginary_quantity = max(
        imaginary_ratio_by_quantity,
        key=imaginary_ratio_by_quantity.__getitem__,
    )
    maximum_imaginary_ratio = float(
        imaginary_ratio_by_quantity[
            maximum_imaginary_quantity
        ]
    )
    ...
    mechanism = {
        ...
        "operator_identity_residuals": {
            ...
            "SHADOW_PS_ADVECTIVE_UNPROJECTED_RC_NYQUIST_V2": 0.0,
            "SHADOW_PS_ADVECTIVE_PROJECTED_RC_NYQUIST_V2": (
...
def compute_single_shadow_transport(
    ...
    *,
    ...
    if operator_id == "BASE_FD_ADVECTIVE_V1":
        ...
        if operator_id == "SHADOW_PS_ADVECTIVE_UNPROJECTED_RC_NYQUIST_V2":
            result = raw
        else:
            result = project_field_with_imaginary(
                raw,
...
def build_checkpoint_route_diagnostics(
    solver: object,
    state: np.ndarray,
    u: np.ndarray,
    v: np.ndarray,
    *,
    ...
    omega_x, omega_y = derivative_pair_complex(
        state_array,
        kx_route,
        ky_route,
    )
    ...
    projected_baseline_complex = np.fft.ifft2(
        np.fft.fft2(baseline_transport) * deal
    )
    projected_baseline_measurement = complex_measurement(
        "projected_baseline_transport_imaginary_ratio",
    ...
    projected_pseudo_complex = np.fft.ifft2(
        np.fft.fft2(pseudo_raw_transport) * deal
    )
    projected_pseudo_measurement = complex_measurement(
        "projected_pseudo_transport_imaginary_ratio",
        {"projection": projected_pseudo_complex},
    )

    u_x, u_y = derivative_pair_complex(
        u,
        kx_route,
        ky_route,
    )
    ...
    v_x, v_y = derivative_pair_complex(
        v,
        kx_route,
        ky_route,
    )
    ...
    return {
        ...
        "projected_baseline_transport": np.asarray(
        ...
        "projected_pseudo_transport": np.asarray(
...
def checkpoint_quantity_real_fields(
    diagnostics: Mapping[str, object],
) -> dict[str, np.ndarray]:
    return {
        "omega_gradient_imaginary_ratio": np.stack(
            (
                np.asarray(diagnostics["omega_x"]).real,
                np.asarray(diagnostics["omega_y"]).real,
            ),
            axis=0,
        ),
        "projected_baseline_transport_imaginary_ratio": (
        ...
        "u_x_gradient_imaginary_ratio": np.stack(
            (
                np.asarray(diagnostics["u_x"]).real,
                np.asarray(diagnostics["u_y"]).real,
            ),
            axis=0,
        ),
        "v_y_gradient_imaginary_ratio": np.stack(
            (
                np.asarray(diagnostics["v_x"]).real,
                np.asarray(diagnostics["v_y"]).real,
            ),
            axis=0,
        ),
    }
...
def build_checkpoint_trace_rows(
    solver: object,
    baseline: Mapping[str, object],
    raw_diagnostics: Mapping[str, object],
    rc_diagnostics: Mapping[str, object],
    *,
    stage: int,
    ...
    raw_fields = checkpoint_quantity_real_fields(
        raw_diagnostics
    )
    rc_fields = checkpoint_quantity_real_fields(
        rc_diagnostics
    )
    raw_by_id = raw_diagnostics["measurement_by_id"]
    rc_by_id = rc_diagnostics["measurement_by_id"]
    assert isinstance(raw_by_id, Mapping)
    assert isinstance(rc_by_id, Mapping)

    source_field_by_quantity = {
        "omega_gradient_imaginary_ratio": state,
        "projected_baseline_transport_imaginary_ratio": (
            np.asarray(
                raw_diagnostics["baseline_transport"],
                dtype=np.float64,
            )
        ),
        "projected_pseudo_transport_imaginary_ratio": (
            np.asarray(
                raw_diagnostics["pseudo_raw_transport"],
                dtype=np.float64,
            )
        ),
        "u_x_gradient_imaginary_ratio": np.asarray(
            raw_diagnostics["u"],
            dtype=np.float64,
        ),
        "v_y_gradient_imaginary_ratio": np.asarray(
            raw_diagnostics["v"],
            dtype=np.float64,
        ),
    }
    ...
    for quantity_id in (
        ...
        x_direction = direction.startswith("x")
        relevant_mask = (
            x_nyquist_mask
            if x_direction
            else y_nyquist_mask
        ...
        source_field = source_field_by_quantity[
            quantity_id
        ]
        ...
        raw_derivative_spectrum = (
            1j * raw_multiplier * source_spectrum
        )
        rc_derivative_spectrum = (
            1j * rc_multiplier * source_spectrum
        )
        ...
        for route, measurement in (
            ...
            row = {
                **common,
                ...
                "dominant_direction": str(
                    measurement["dominant_direction"]
                ),
                "real_rms": float(
                ...
                "imaginary_max_abs": float(
                    measurement["imaginary_max_abs"]
                ),
                "ratio_denominator": float(
                    measurement["ratio_denominator"]
                ),
                ...
                "is_historical_failure": (
                    loop_index
                    == EXPECTED_HISTORICAL_RAW_LOOP_INDEX
                    and stage
                    == EXPECTED_HISTORICAL_RAW_STAGE
                    and route == "raw"
                    and quantity_id
                    == EXPECTED_HISTORICAL_RAW_QUANTITY
                ),
            ...
            if not all_numeric_values_finite(row):
                raise IntegrityFailure(
                    "checkpoint_trace_row_finite",
                    "checkpoint trace row contains "
                    "nonfinite data",
                    stage=str(stage),
                )
...
def build_checkpoint_work_row(
    *,
    ...
    work_relative = abs(rc_work - raw_work) / max(
        abs(raw_work),
        abs(rc_work),
        RESIDUAL_FLOOR,
    )
    raw_nonzero = (
        abs(raw_work)
        > CHECKPOINT_WORK_ABSOLUTE_LIMIT
    )
    rc_nonzero = (
        abs(rc_work)
        > CHECKPOINT_WORK_ABSOLUTE_LIMIT
    ...
    near_zero_changed = raw_nonzero != rc_nonzero
    material_change = (
        difference_relative
        > CHECKPOINT_TRANSPORT_RELATIVE_LIMIT
        or (
            work_absolute
            > CHECKPOINT_WORK_ABSOLUTE_LIMIT
            and work_relative
            > CHECKPOINT_WORK_RELATIVE_LIMIT
        )
        or sign_changed
    ...
    row = {
        ...
        "stage": stage,
        "operator_id": operator_id,
        "historical_raw_transport_rms": raw_rms,
        "rc_transport_rms": rc_rms,
        "transport_difference_rms": difference_rms,
        "transport_difference_max_abs": (
            max_abs(difference)
        ),
        "transport_difference_relative": (
        ...
        "historical_raw_work": raw_work,
        "rc_work": rc_work,
        "work_absolute_difference": work_absolute,
        "work_relative_difference": work_relative,
    ...
    if not all_numeric_values_finite(row):
        raise IntegrityFailure(
            "checkpoint_work_row_finite",
            "checkpoint work row contains nonfinite data",
        )

    return row


def build_checkpoint_work_rows(
    baseline: Mapping[str, object],
    ...
    for stage_number, raw, rc in (
        ...
        for operator_id, transport_key in (
            (
                PSEUDO_RC_UNPROJECTED_ID,
                "pseudo_raw_transport",
            ),
    ...
    for operator_id in PSEUDO_RC_OPERATOR_IDS:
        raw_transport = 0.5 * (
...
def verify_checkpoint_rows(
    ...
    if len(trace_rows) != EXPECTED_CHECKPOINT_TRACE_ROWS:
        raise IntegrityFailure(
            "checkpoint_trace_row_count",
            f"trace rows={len(trace_rows)}, "
            f"expected={EXPECTED_CHECKPOINT_TRACE_ROWS}",
        )

    if len(work_rows) != EXPECTED_CHECKPOINT_WORK_ROWS:
        raise IntegrityFailure(
            "checkpoint_work_row_count",
            f"work rows={len(work_rows)}, "
            f"expected={EXPECTED_CHECKPOINT_WORK_ROWS}",
        )

    raw_failure = next(
        row
        for row in trace_rows
        if bool(row["is_historical_failure"])
    )
    rc_failure = next(
        row
        for row in trace_rows
        if (
            ...
            and int(row["stage"])
            == EXPECTED_HISTORICAL_RAW_STAGE
            and row["route"] == "real_compatible"
            and row["quantity_id"]
            == EXPECTED_HISTORICAL_RAW_QUANTITY
        )
    )

    raw_ratio = float(raw_failure["imaginary_ratio"])
    rc_ratio = float(rc_failure["imaginary_ratio"])

    raw_pass = (
        raw_ratio > IMAGINARY_RATIO_LIMIT
        and math.isclose(
            raw_ratio,
            EXPECTED_HISTORICAL_RAW_RATIO,
            rel_tol=RAW_CHECKPOINT_RELATIVE_TOLERANCE,
            abs_tol=RAW_CHECKPOINT_ABSOLUTE_TOLERANCE,
        )
    )
    rc_pass = (
        rc_ratio <= IMAGINARY_RATIO_LIMIT
        and math.isclose(
            rc_ratio,
            EXPECTED_REAL_COMPATIBLE_RATIO,
            rel_tol=RC_CHECKPOINT_RELATIVE_TOLERANCE,
            abs_tol=RC_CHECKPOINT_ABSOLUTE_TOLERANCE,
        )
    )

    if not raw_pass:
        raise IntegrityFailure(
            "historical_raw_checkpoint",
            f"historical raw ratio={raw_ratio}, "
            f"expected={EXPECTED_HISTORICAL_RAW_RATIO}",
            stage=str(EXPECTED_HISTORICAL_RAW_STAGE),
        )

    if not rc_pass:
        raise IntegrityFailure(
            "real_compatible_checkpoint",
            f"real-compatible ratio={rc_ratio}, "
            f"expected={EXPECTED_REAL_COMPATIBLE_RATIO}",
            stage=str(EXPECTED_HISTORICAL_RAW_STAGE),
        )

    maximum_derivative_relative_difference = max(
        float(
            row[
                "raw_to_rc_real_difference_relative"
            ]
        ...
        for row in trace_rows
        if row["route"] == "real_compatible"
    )
    maximum_transport_relative_difference = max(
        float(row["transport_difference_relative"])
        for row in work_rows
    )
    maximum_work_absolute_difference = max(
        float(row["work_absolute_difference"])
        for row in work_rows
    )

    if (
        maximum_derivative_relative_difference
        > CHECKPOINT_REAL_DERIVATIVE_RELATIVE_LIMIT
    ):
        raise IntegrityFailure(
            "checkpoint_real_derivative_preservation",
            "checkpoint real derivative difference "
            f"={maximum_derivative_relative_difference}",
        )

    if (
        maximum_transport_relative_difference
        > CHECKPOINT_TRANSPORT_RELATIVE_LIMIT
    ):
        raise IntegrityFailure(
            "checkpoint_transport_preservation",
            "checkpoint transport difference "
            f"={maximum_transport_relative_difference}",
        )

    if any(
        bool(row["material_real_work_change"])
        for row in work_rows
    ):
        raise IntegrityFailure(
            "checkpoint_work_preservation",
            "checkpoint reports material real-work change",
        )

    current_hash = sha256_array(
        baseline["current"]
    )
    stage_hash = sha256_array(
        baseline["stage"]
    )
    filtered_hash = sha256_array(
        baseline["filtered"]
    )

    if current_hash != EXPECTED_CHECKPOINT_CURRENT_SHA256:
        raise IntegrityFailure(
            "checkpoint_current_state_sha256",
            f"current state SHA256={current_hash}",
        )

    if stage_hash != EXPECTED_CHECKPOINT_STAGE_SHA256:
        raise IntegrityFailure(
            "checkpoint_stage_state_sha256",
            f"stage state SHA256={stage_hash}",
        )

    if filtered_hash != EXPECTED_CHECKPOINT_FILTERED_SHA256:
        raise IntegrityFailure(
            "checkpoint_filtered_state_sha256",
            f"filtered state SHA256={filtered_hash}",
        )

    return {
        "historical_raw_checkpoint_pass": True,
        "real_compatible_checkpoint_pass": True,
        "loop_index": EXPECTED_HISTORICAL_RAW_LOOP_INDEX,
        "stage": EXPECTED_HISTORICAL_RAW_STAGE,
        "quantity_id": EXPECTED_HISTORICAL_RAW_QUANTITY,
        "raw_ratio": raw_ratio,
        "real_compatible_ratio": rc_ratio,
        "maximum_real_derivative_relative_difference": (
            maximum_derivative_relative_difference
        ),
        "maximum_transport_relative_difference": (
            maximum_transport_relative_difference
        ),
        "maximum_work_absolute_difference": (
            maximum_work_absolute_difference
        ),
        "current_state_sha256": current_hash,
        "stage_state_sha256": stage_hash,
        "filtered_state_sha256": filtered_hash,
    }
...
def build_shadow_rows(
    ...
    for registry_item in OPERATOR_REGISTRY:
        ...
        row = {
            ...
            "derivative_convention": registry_item[
                "derivative_convention"
            ],
...
def classify_time_block(
    ...
    if all(
        bool(block[operator_id]["near_neutral_pass"])
        for operator_id in PRIMARY_ALTERNATE_IDS
    ):
        return (
            "ALL PRIMARY REMEDIATED ALTERNATES "
            "NEAR-NEUTRAL IN BLOCK"
        )
    ...
    persistent_families = {
        ...
        for operator_id in PRIMARY_ALTERNATE_IDS
        if bool(
            block[operator_id]["persistence_pass"]
        )
    }

    if len(persistent_families) >= 2:
        return (
            "NONZERO WORK PERSISTS ACROSS MULTIPLE "
            "REMEDIATED FAMILIES IN BLOCK"
        )
    ...
    persistence_any = any(
        bool(block[operator_id]["persistence_pass"])
        for operator_id in PRIMARY_ALTERNATE_IDS
    )
    intermediate_any = any(
        not bool(
            block[operator_id]["near_neutral_pass"]
        )
        and not bool(
            block[operator_id]["persistence_pass"]
        )
    ...
    if near_neutral_any and (
        persistence_any or intermediate_any
    ):
        return (
            "REMEDIATED SHADOW RESPONSE "
            "FORM-DEPENDENT AND MIXED IN BLOCK"
        )

    if not near_neutral_any:
        return (
            "REMEDIATED SHADOW SET SHOWS NO "
            "STRONG SEPARATION IN BLOCK"
        )

    return "REMEDIATED SHADOW BLOCK INCONCLUSIVE"
...
def classify_shadow_response(
    ...
    if all_near_neutral:
        return "REMEDIATED SHADOW SET SUPPORTS CURRENT-FORM SPECIFICITY"
    ...
    if len(persistent_families) >= 2:
        return "NONZERO WORK PERSISTS ACROSS MULTIPLE REMEDIATED SHADOW FAMILIES"
    ...
    if near_neutral_any and (persistent_any or intermediate_any):
        return "REMEDIATED SHADOW RESPONSE IS FORM-DEPENDENT AND MIXED"

    if not near_neutral_any:
        return "REMEDIATED SHADOW SET DOES NOT SHOW STRONG FORM SEPARATION"

    return "REMEDIATED SAME-STATE SHADOW AUDIT INCONCLUSIVE"
    ...
    *,
    ...
    lines = [
        ...
        (
            ...
            f"`{checkpoint_summary['historical_raw_checkpoint_pass']}`"
        ),
        (
            "- Real-compatible checkpoint: "
            f"`{checkpoint_summary['real_compatible_checkpoint_pass']}`"
        ),
        (
            "- Checkpoint loop index: "
            f"`{checkpoint_summary['loop_index']}`"
        ),
        (
            "- Checkpoint stage: "
            f"`{checkpoint_summary['stage']}`"
        ),
        (
            "- Historical raw ratio: "
            f"`{checkpoint_summary['raw_ratio']:.16e}`"
        ),
        (
            "- Real-compatible ratio: "
            f"`{checkpoint_summary['real_compatible_ratio']:.16e}`"
        ),
        (
            "- Maximum checkpoint real-derivative relative difference: "
            f"`{checkpoint_summary['maximum_real_derivative_relative_difference']:.16e}`"
        ),
        (
            "- Maximum checkpoint transport relative difference: "
            f"`{checkpoint_summary['maximum_transport_relative_difference']:.16e}`"
        ),
        (
            "- Maximum checkpoint work absolute difference: "
            f"`{checkpoint_summary['maximum_work_absolute_difference']:.16e}`"
        ),
        "",
        ...
        lines.append(
            ...
            + " | ".join(
                (
                    f"`{block_id}`",
                    f"`{block['label']}`",
                    f"`{classify_time_block(structured[block_id])}`",
                )
            )
            + " |"
        )
...
def _is_solver_wavenumber_attribute(
    node: ast.AST,
) -> bool:
    return (
        ...
        and node.attr in {"kx", "ky"}
    )


def _has_checkpoint_guard(
    node: ast.AST,
    parent_map: Mapping[ast.AST, ast.AST],
) -> bool:
    ...
    while current is not None:
        if isinstance(current, ast.If):
            test_text = ast.unparse(current.test)

            if (
                "loop_index in NYQUIST_CHECKPOINT_LOOP_INDICES"
                in test_text
            ):
                return True
...
def inspect_ast(source: str) -> dict[str, object]:
    ...
    raw_checkpoint_calls = 0
    unguarded_raw_checkpoint_calls = 0
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            if name == "build_checkpoint_route_diagnostics":
                route_keywords = [
                    keyword
                ...
                if (
                    len(route_keywords) == 1
                    and isinstance(
                        route_keywords[0].value,
                        ast.Constant,
                    )
                    and route_keywords[0].value.value
                    == "raw"
                ):
                    raw_checkpoint_calls += 1

                    if not _has_checkpoint_guard(
                        node,
                        parent_map,
                    ):
                        unguarded_raw_checkpoint_calls += 1
            ...
            for target in targets:
                if _is_solver_wavenumber_attribute(target):
    ...
    if invalid_imports:
        raise RuntimeError(
            "project imports outside execute_audit: "
            f"{invalid_imports}"
        )
    ...
    if forbidden_calls:
        raise RuntimeError(
            f"forbidden call sites present: "
            f"{forbidden_calls}"
        )
    ...
    if solver_wavenumber_assignments:
        raise RuntimeError(
            "solver wavenumber assignments are present: "
            f"{solver_wavenumber_assignments}"
        )

    if raw_checkpoint_calls != 2:
        raise RuntimeError(
            f"historical raw checkpoint calls="
            f"{raw_checkpoint_calls}, expected=2"
        )

    if unguarded_raw_checkpoint_calls != 0:
        raise RuntimeError(
            "historical raw route is not confined to "
    ...
    if forbidden_identifiers:
        raise RuntimeError(
            "forbidden alternate-trajectory identifiers "
            f"present: {forbidden_identifiers}"
        )
    ...
    if baseline_assignments < 2:
        raise RuntimeError(
            "baseline_omega does not have the expected "
            "initialization and accepted-state assignment"
        )
    ...
    for function_name in (
        ...
        if function is None:
            raise RuntimeError(
                f"required function is missing: "
                f"{function_name}"
            )
        ...
        forbidden_references = [
            ...
            if _is_solver_wavenumber_attribute(node)
        ]

        if forbidden_references:
            raise RuntimeError(
                f"{function_name} contains full-run raw "
                f"solver wavenumber references: "
    ...
    required_source_fragments = (
        "kx_real_compatible = np.array(",
        "ky_real_compatible = np.array(",
        "kx_shadow=kx_real_compatible",
        "ky_shadow=ky_real_compatible",
        "loop_index in NYQUIST_CHECKPOINT_LOOP_INDICES",
    ...
    if missing_fragments:
        raise RuntimeError(
            f"required remediated source fragments "
            f"are missing: {missing_fragments}"
        )
    ...
    return {
        ...
        "baseline_omega_assignments": (
            baseline_assignments
        ),
        "raw_checkpoint_calls": raw_checkpoint_calls,
        "unguarded_raw_checkpoint_calls": (
            unguarded_raw_checkpoint_calls
        ),
...
def inspect_runner(repo: Path) -> int:
    ...
    if len(OPERATOR_REGISTRY) != 7:
        fail(
            f"operator count={len(OPERATOR_REGISTRY)}, "
            "expected=7"
        )

    if len(PRIMARY_ALTERNATE_IDS) != 5:
        fail(
            f"primary alternate count="
            f"{len(PRIMARY_ALTERNATE_IDS)}, expected=5"
        )
    ...
    expected_operator_ids = (
        ...
        "SHADOW_FD_SKEW_V1",
        (
            "SHADOW_PS_ADVECTIVE_"
            "UNPROJECTED_RC_NYQUIST_V2"
        ),
        (
            "SHADOW_PS_ADVECTIVE_"
            "PROJECTED_RC_NYQUIST_V2"
        ),
        "SHADOW_ARAKAWA_V1",
    )
    ...
    if OPERATOR_IDS != expected_operator_ids:
        fail(
            "operator registry differs from frozen "
            f"remediated design: {OPERATOR_IDS!r}"
        )

    if PRIMARY_ALTERNATE_IDS != (
        "SHADOW_FD_CONSERVATIVE_V1",
        "SHADOW_FD_SKEW_V1",
        (
            "SHADOW_PS_ADVECTIVE_"
        ...
        fail(
            "primary alternate registry differs from "
            "the frozen remediated design"
        )
    ...
    if expected_thresholds != (
        ...
        fail(
            "classification thresholds differ from "
            "the frozen design"
        )
    ...
    expected_integrity_limits = (
        ...
        SPECTRAL_DIVERGENCE_LIMIT,
        IMAGINARY_RATIO_LIMIT,
        RAW_CHECKPOINT_RELATIVE_TOLERANCE,
        RAW_CHECKPOINT_ABSOLUTE_TOLERANCE,
        RC_CHECKPOINT_RELATIVE_TOLERANCE,
        RC_CHECKPOINT_ABSOLUTE_TOLERANCE,
        CHECKPOINT_REAL_DERIVATIVE_RELATIVE_LIMIT,
        CHECKPOINT_TRANSPORT_RELATIVE_LIMIT,
        CHECKPOINT_WORK_ABSOLUTE_LIMIT,
        CHECKPOINT_WORK_RELATIVE_LIMIT,
    ...
    if expected_integrity_limits != (
        ...
        fail(
            "integrity tolerances differ from "
            "the frozen remediated design"
        )

    if NYQUIST_CHECKPOINT_LOOP_INDICES != (
        3058,
        3059,
    ):
        fail(
            "Nyquist checkpoint index set differs "
            "from frozen design"
        )
    ...
    if SENTINEL_LOOP_INDICES != (
        ...
        fail(
            "sentinel index set differs from "
            "the frozen remediated design"
        )

    if (
        EXPECTED_SHADOW_ROWS
        != STEPS * len(OPERATOR_REGISTRY)
    ):
    ...
    if EXPECTED_TIME_BLOCK_ROWS != (
        ...
        fail(
            "expected time-block-row formula "
            "is incorrect"
        )

    if EXPECTED_CHECKPOINT_TRACE_ROWS != (
        len(NYQUIST_CHECKPOINT_LOOP_INDICES)
        * 2
        * 2
        * 5
    ):
        fail(
            "expected checkpoint trace-row formula "
            "is incorrect"
        )

    if EXPECTED_CHECKPOINT_WORK_ROWS != 6:
        fail(
            "expected checkpoint work-row count "
            "is incorrect"
        )

    if len(OUTPUT_FILENAMES) != 9:
        fail(
            f"output file count={len(OUTPUT_FILENAMES)}, "
            "expected=9"
        )
    ...
    print("=" * 72)
    print(
        "STAGE C REMEDIATED FULL SAME-STATE "
    ...
    print(
        "Remediated full design SHA256:",
        source_hashes["remediated_full_design"],
    )
    print(
        "Focused remediation completion-report SHA256:",
        source_hashes["remediation_completion_report"],
    )
    print(
        "Configuration: N64, Re1000, "
        "dt0.005, steps20001"
    )
    print(
    ...
    print(
        "Primary alternate operators:",
        len(PRIMARY_ALTERNATE_IDS),
    )
    print(
    ...
    print(
        "Expected time-block rows:",
        EXPECTED_TIME_BLOCK_ROWS,
    )
    print(
        "Expected Nyquist checkpoint trace rows:",
        EXPECTED_CHECKPOINT_TRACE_ROWS,
    )
    print(
        "Expected Nyquist checkpoint work rows:",
        EXPECTED_CHECKPOINT_WORK_ROWS,
    )
    print(
        "Expected archived comparisons:",
        EXPECTED_ARCHIVE_MATCHES,
    )
    print(
        "Historical raw route confined to loops "
        "3058 and 3059: PASS"
    )
    print(
        "Full-run pseudo-spectral derivative:",
        "REAL-COMPATIBLE NYQUIST-ZEROED",
    )
    ...
    print(
        "Same baseline current state for shadows: "
        "PRESENT"
    )
    print(
        "Same baseline RK2 stage state for shadows: "
        "PRESENT"
    )
    print(
        "Sentinel forward/reverse order gate: "
        "PRESENT"
    )
    print(
        "Centered product-rule diagnostics: PRESENT"
    )
    print(
        "RC pseudo-spectral projection diagnostics: "
        "PRESENT"
    )
    ...
    print(
        "Remediated full Stage C numerical "
        "execution authorized by inspection: NO"
    )
...
def execute_audit(repo: Path) -> int:
    ...
    state_rows = 0
    shadow_rows_count = 0
    checkpoint_trace_rows: list[
        dict[str, object]
    ] = []
    checkpoint_work_rows: list[
        dict[str, object]
    ] = []
    ...
    metadata: dict[str, object] = {
        "schema_id": (
            "STAGE_C_REMEDIATED_FULL_SAME_STATE_SHADOW_METADATA_V1"
        ),
        "run_id": run_id,
        "status": "running",
        "classification": None,
        ...
        "configuration": {
            ...
            "stage_state_policy": STAGE_STATE_POLICY,
            "full_run_shadow_derivative_convention": (
                "operator_registry_specific"
            ),
            ...
            "full_run_pseudo_spectral_derivative": (
                "real_compatible_nyquist_zeroed"
            ),
            "historical_raw_route": (
                "checkpoint_only_3058_3059"
            ),
        },
        "operator_registry": [
            dict(item)
        ...
        "output_files": {
            ...
            "remediated_shadow_state_reference": (
                state_path.name
            ),
            "remediated_shadow_advection_per_step": (
                shadow_path.name
            ),
            "remediated_shadow_time_blocks": (
                block_path.name
            ),
            "nyquist_remediation_checkpoint_trace": (
                checkpoint_trace_path.name
            ),
            "nyquist_remediation_checkpoint_work": (
                checkpoint_work_path.name
            ),
            "remediated_shadow_summary": (
                summary_path.name
            ),
    ...
    try:
        ...
        kx_real_compatible_hash = sha256_array(
            kx_real_compatible
        )
        ky_real_compatible_hash = sha256_array(
            ky_real_compatible
        )
        ...
        for loop_index in range(STEPS):
            ...
            if loop_index in NYQUIST_CHECKPOINT_LOOP_INDICES:
                raw_checkpoint_stage1 = (
                    build_checkpoint_route_diagnostics(
                        solver,
                        current,
                        np.asarray(baseline["u_1"]),
                        np.asarray(baseline["v_1"]),
                ...
                checkpoint_trace_rows.extend(
                    build_checkpoint_trace_rows(
                        solver,
                        baseline,
                        raw_checkpoint_stage1,
                        rc_checkpoint_stage1,
                        stage=1,
                        x_nyquist_mask=x_nyquist_mask,
                        y_nyquist_mask=y_nyquist_mask,
                        kx_rc=kx_real_compatible,
                        ky_rc=ky_real_compatible,
                    )
                )
                checkpoint_trace_rows.extend(
                    build_checkpoint_trace_rows(
                ...
                if (
                    ...
                    print(
                        "historical raw checkpoint: PASS",
                        f"ratio={checkpoint_summary['raw_ratio']:.16e}",
                    )
                    print(
                        "real-compatible checkpoint: PASS",
                        (
            ...
            if (
                ...
                print(
                    ...
                    (
                        "Rpsp_rc="
                        f"{float(shadow_rows[5]['stage_weighted_rhs_work']):.6e}"
                    ),
                    (
                        "Rarakawa="
                    ...
                    (
                        "max_imag="
                        f"{max(float(stage1_mechanism['maximum_imaginary_ratio']), float(stage2_mechanism['maximum_imaginary_ratio'])):.6e}"
                    ),
                )
        ...
        if checkpoint_summary is None:
            raise IntegrityFailure(
                "nyquist_checkpoint_missing",
                "historical Nyquist checkpoint was not verified",
            )

        if (
            len(checkpoint_trace_rows)
            != EXPECTED_CHECKPOINT_TRACE_ROWS
        ):
            raise IntegrityFailure(
                "checkpoint_trace_row_count",
                f"trace rows={len(checkpoint_trace_rows)}, "
                f"expected={EXPECTED_CHECKPOINT_TRACE_ROWS}",
            )

        if (
            len(checkpoint_work_rows)
            != EXPECTED_CHECKPOINT_WORK_ROWS
        ):
            raise IntegrityFailure(
                "checkpoint_work_row_count",
                f"work rows={len(checkpoint_work_rows)}, "
                f"expected={EXPECTED_CHECKPOINT_WORK_ROWS}",
            )
        ...
        if (
            sha256_array(kx_real_compatible)
            != kx_real_compatible_hash
            or sha256_array(ky_real_compatible)
            != ky_real_compatible_hash
        ):
            raise IntegrityFailure(
                "real_compatible_wavenumber_mutation",
                "local real-compatible wavenumber arrays changed",
            )
        ...
        summary = {
            "schema_id": (
                "STAGE_C_REMEDIATED_FULL_SAME_STATE_SHADOW_SUMMARY_V1"
            ),
            "run_id": run_id,
            ...
            "same_state_policy": {
                ...
                "alternate_trajectories_executed": False,
                "full_run_pseudo_spectral_route": (
                    "real_compatible_nyquist_zeroed"
                ),
                "historical_raw_route_scope": (
                    "loop_indices_3058_and_3059_only"
                ),
            },
            "counts": {
                "state_reference_rows": state_rows,
                ...
                "checkpoint_trace_rows": len(
                    checkpoint_trace_rows
                ),
                "checkpoint_work_rows": len(
                    checkpoint_work_rows
                ),
            ...
            "nyquist_remediation_checkpoint": (
                checkpoint_summary
            ),
            "time_block_classifications": {
                str(block["block_id"]): (
                    classify_time_block(
                        structured[
                            str(block["block_id"])
        ...
        print(
            "Nyquist checkpoint trace rows:",
            len(checkpoint_trace_rows),
        )
        print(
            "Nyquist checkpoint work rows:",
            len(checkpoint_work_rows),
        )
        print("Historical raw checkpoint: PASS")
        print("Real-compatible checkpoint: PASS")
        ...
        print()
        print("STAGE C REMEDIATED FULL SAME-STATE SHADOW AUDIT: FAILED")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "single controlled remediated full same-state audit"
        ),
    )
