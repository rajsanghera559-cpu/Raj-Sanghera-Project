...
OUT_CSV = Path("PHASE9A4_HALF_DT_NONLINEAR_DRIFT_AUDIT.csv")
...
print("\n=== PHASE 9A.4 HALF-DT NONLINEAR DRIFT AUDIT ===")
...
for path in [METADATA_PATH, INITIAL_PATH, DIAG_PATH, SPEC_PATH, REF_9A3_CSV]:
    if not path.exists():
...
comparison_intervals = int(config.get("comparison_intervals"))
...
metadata_commit_expected = str(git_commit).startswith("1c1cd19")
metadata_mode_expected = mode == "phase9A4_nonlinear_no_forcing_drift_dt_half"
...
reference_final_energy = float(ref9a3["final_energy"].iloc[0])
reference_final_enstrophy = float(ref9a3["final_enstrophy"].iloc[0])
reference_final_E_k4 = float(ref9a3["final_E_k4"].iloc[0])
reference_energy_change = float(ref9a3["energy_change_initial_to_final"].iloc[0])
reference_enstrophy_change = float(ref9a3["enstrophy_change_initial_to_final"].iloc[0])
reference_k3_fraction = float(ref9a3["k3_fraction"].iloc[0])
reference_k4_fraction = float(ref9a3["k4_fraction"].iloc[0])
reference_kge5_fraction = float(ref9a3["kge5_fraction"].iloc[0])

final_energy_matches_9A3 = relative_error(final_energy, reference_final_energy) < 1e-8
final_enstrophy_matches_9A3 = relative_error(final_enstrophy, reference_final_enstrophy) < 1e-8
final_E_k4_matches_9A3 = relative_error(final_E_k4, reference_final_E_k4) < 1e-6

energy_drift_consistent_with_9A3 = abs(energy_change_initial_to_final - reference_energy_change) < 1e-7
enstrophy_drift_consistent_with_9A3 = abs(enstrophy_change_initial_to_final - reference_enstrophy_change) < 1e-7

energy_drift_not_worse_than_9A3 = abs(energy_change_initial_to_final) <= abs(reference_energy_change) * 1.10
enstrophy_drift_not_worse_than_9A3 = abs(enstrophy_change_initial_to_final) <= abs(reference_enstrophy_change) * 1.10

k3_fraction_matches_9A3 = abs(k3_fraction - reference_k3_fraction) < 1e-6
k4_fraction_matches_9A3 = abs(k4_fraction - reference_k4_fraction) < 1e-6
kge5_fraction_matches_9A3 = abs(kge5_fraction - reference_kge5_fraction) < 1e-6
...
overall_pass = all(
    [
        ...
        initial_peak_valid,
        final_peak_valid,
        final_energy_matches_9A3,
        final_enstrophy_matches_9A3,
        energy_drift_consistent_with_9A3,
        enstrophy_drift_consistent_with_9A3,
        energy_drift_not_worse_than_9A3,
        enstrophy_drift_not_worse_than_9A3,
        k3_fraction_matches_9A3,
        k4_fraction_matches_9A3,
        kge5_fraction_matches_9A3,
    ]
)
...
print(f"Git commit starts with 1c1cd19: {pass_fail(metadata_commit_expected)}")
...
print("\n=== PHASE 9A.3 COMPARISON ===")
print(f"Reference 9A.3 final energy: {reference_final_energy:.12e}")
print(f"Phase 9A.4 final energy:     {final_energy:.12e}")
print(f"Final energy matches 9A.3:   {pass_fail(final_energy_matches_9A3)}")
print(f"Reference 9A.3 final enstrophy: {reference_final_enstrophy:.12e}")
print(f"Phase 9A.4 final enstrophy:     {final_enstrophy:.12e}")
print(f"Final enstrophy matches 9A.3:   {pass_fail(final_enstrophy_matches_9A3)}")
print(f"Reference 9A.3 energy drift: {reference_energy_change:.12e}")
print(f"Phase 9A.4 energy drift:     {energy_change_initial_to_final:.12e}")
print(f"Energy drift consistent:     {pass_fail(energy_drift_consistent_with_9A3)}")
print(f"Energy drift not worse:      {pass_fail(energy_drift_not_worse_than_9A3)}")
print(f"Reference 9A.3 enstrophy drift: {reference_enstrophy_change:.12e}")
print(f"Phase 9A.4 enstrophy drift:     {enstrophy_change_initial_to_final:.12e}")
print(f"Enstrophy drift consistent:     {pass_fail(enstrophy_drift_consistent_with_9A3)}")
print(f"Enstrophy drift not worse:      {pass_fail(enstrophy_drift_not_worse_than_9A3)}")
print(f"k=3 fraction matches 9A.3:      {pass_fail(k3_fraction_matches_9A3)}")
print(f"k=4 fraction matches 9A.3:      {pass_fail(k4_fraction_matches_9A3)}")
print(f"k>=5 fraction matches 9A.3:     {pass_fail(kge5_fraction_matches_9A3)}")

print("\n=== OVERALL RESULT ===")
print(f"Phase 9A.4 half-dt nonlinear drift audit: {pass_fail(overall_pass)}")
...
summary = {
    ...
    "reference_9A3_final_energy": reference_final_energy,
    "reference_9A3_final_enstrophy": reference_final_enstrophy,
    "reference_9A3_final_E_k4": reference_final_E_k4,
    "reference_9A3_energy_change_initial_to_final": reference_energy_change,
    "reference_9A3_enstrophy_change_initial_to_final": reference_enstrophy_change,
    "final_energy_matches_9A3": pass_fail(final_energy_matches_9A3),
    "final_enstrophy_matches_9A3": pass_fail(final_enstrophy_matches_9A3),
    "final_E_k4_matches_9A3": pass_fail(final_E_k4_matches_9A3),
    "energy_drift_consistent_with_9A3": pass_fail(energy_drift_consistent_with_9A3),
    "enstrophy_drift_consistent_with_9A3": pass_fail(enstrophy_drift_consistent_with_9A3),
    "energy_drift_not_worse_than_9A3": pass_fail(energy_drift_not_worse_than_9A3),
    "enstrophy_drift_not_worse_than_9A3": pass_fail(enstrophy_drift_not_worse_than_9A3),
    "k3_fraction_matches_9A3": pass_fail(k3_fraction_matches_9A3),
    "k4_fraction_matches_9A3": pass_fail(k4_fraction_matches_9A3),
    "kge5_fraction_matches_9A3": pass_fail(kge5_fraction_matches_9A3),
...
print("Phase 9A.4 half-dt nonlinear drift audit complete.")