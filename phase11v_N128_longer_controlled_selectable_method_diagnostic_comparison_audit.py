...
METHOD_OUT_CSV = Path("PHASE11V_N128_LONGER_CONTROLLED_SELECTABLE_METHOD_DIAGNOSTIC_COMPARISON_AUDIT.csv")
PAIRWISE_OUT_CSV = Path("PHASE11V_N128_LONGER_CONTROLLED_SELECTABLE_METHOD_DIAGNOSTIC_COMPARISON_PAIRWISE.csv")
...
print("Purpose: compare fd_centered, pseudo_spectral, and arakawa at N=128 through final time 1.0.")
...
print("This does not prove turbulence, k^-3 scaling, convergence, or method superiority.")

reference_solver = SpectralSolver(
    nx=N,
    ny=N,
    Re=RE,
    run_path=Path("experiments") / "reference" / "phase11V_N128_reference_grid",
    dt=DT,
    steps=STEPS,
)
...
for method in METHODS:
    solvers[method] = SelectableAdvectionSolver(
        ...
        ny=N,
        Re=RE,
        run_path=Path("experiments") / "selectable_diagnostics" / f"phase11V_N128_{method}",
        dt=DT,
        steps=STEPS,
...
print("\n=== OVERALL RESULT ===")
print(f"Phase 11V N128 longer controlled selectable method diagnostic comparison audit: {pass_fail(overall_pass)}")

print("\n=== OUTPUT WRITTEN ===")
print(f"Wrote: {METHOD_OUT_CSV}")
print(f"Wrote: {PAIRWISE_OUT_CSV}")
print("Phase 11V audit complete.")