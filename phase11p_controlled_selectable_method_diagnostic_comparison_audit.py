...
METHOD_OUT_CSV = Path("PHASE11P_CONTROLLED_SELECTABLE_METHOD_DIAGNOSTIC_COMPARISON_AUDIT.csv")
PAIRWISE_OUT_CSV = Path("PHASE11P_CONTROLLED_SELECTABLE_METHOD_DIAGNOSTIC_COMPARISON_PAIRWISE.csv")
...
ENERGY_RATIO_MIN = 0.01
ENERGY_RATIO_MAX = 100.0

ENSTROPHY_RATIO_MIN = 0.01
ENSTROPHY_RATIO_MAX = 100.0
...
print("Purpose: compare fd_centered, pseudo_spectral, and arakawa diagnostic runs.")
...
reference_solver = SpectralSolver(
    ...
    ny=N,
    Re=RE,
    run_path=Path("experiments") / "reference" / "phase11P_reference_grid",
    dt=DT,
    steps=STEPS,
)
...
for method in METHODS:
    solvers[method] = SelectableAdvectionSolver(
        ...
        ny=N,
        Re=RE,
        run_path=Path("experiments") / "selectable_diagnostics" / f"phase11P_{method}",
        dt=DT,
        steps=STEPS,
...
print("\n=== OVERALL RESULT ===")
print(f"Phase 11P controlled selectable method diagnostic comparison audit: {pass_fail(overall_pass)}")

print("\n=== OUTPUT WRITTEN ===")
print(f"Wrote: {METHOD_OUT_CSV}")
print(f"Wrote: {PAIRWISE_OUT_CSV}")
print("Phase 11P audit complete.")