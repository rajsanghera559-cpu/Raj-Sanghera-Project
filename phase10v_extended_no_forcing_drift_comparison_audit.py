...
OUT_AUDIT_CSV = Path("PHASE10V_EXTENDED_NO_FORCING_DRIFT_COMPARISON_AUDIT.csv")
OUT_TIME_CSV = Path("PHASE10V_EXTENDED_NO_FORCING_DRIFT_TIME_HISTORY.csv")
OUT_PAIRWISE_CSV = Path("PHASE10V_EXTENDED_NO_FORCING_DRIFT_PAIRWISE_SUMMARY.csv")
...
def audit_invalid_method():
    try:
        NoForcingSelectableAdvectionSolver(
            ...
            ny=N,
            Re=RE,
            run_path=Path("experiments") / "method_diagnostics" / "phase10V_invalid",
            dt=DT,
            steps=1,
...
def make_solver(method):
    return NoForcingSelectableAdvectionSolver(
        ...
        ny=N,
        Re=RE,
        run_path=Path("experiments") / "method_diagnostics" / f"phase10V_{method}",
        dt=DT,
        steps=STEPS,
...
print("Purpose: extended no-forcing drift across fd_centered, pseudo_spectral, and arakawa.")
...
print(f"Phase 10V extended no-forcing drift comparison audit: {pass_fail(overall_pass)}")

print("\n=== OUTPUT WRITTEN ===")
print(f"Wrote: {OUT_AUDIT_CSV}")
print(f"Wrote: {OUT_TIME_CSV}")
print(f"Wrote: {OUT_PAIRWISE_CSV}")
print("Phase 10V extended no-forcing drift comparison audit complete.")