...
RUN = Path(r"experiments\runs\run_2026-07-02_14-48-55")
METADATA_PATH = RUN / "metadata.json"
DIAG_PATH = RUN / "diagnostics.csv"
SPEC_PATH = RUN / "spectrum.csv"

OUT_CSV = Path("PHASE7C_FRESH_RUN_AUDIT.csv")
...
metadata_commit_expected = str(git_commit).startswith("7e86077")
metadata_mode_expected = mode == "phase7B_reproducibility_smoke"
metadata_config_expected = (
    ...
    and steps == 1001
    and forcing == "default_solver_forcing"
)
...
single_shell_expected = peak_fraction > 0.999999
...
print(f"Git commit starts with 7e86077: {pass_fail(metadata_commit_expected)}")
...
print(f"Single-shell expected:    {pass_fail(single_shell_expected)}")

print("\n=== OVERALL RESULT ===")
print(f"Phase 7C fresh run audit: {pass_fail(overall_pass)}")
...
summary = {
    ...
    "single_shell_expected": pass_fail(single_shell_expected),
...
print("Phase 7C fresh run audit complete.")