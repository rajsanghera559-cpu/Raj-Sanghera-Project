...
METHOD_OUT_CSV = Path("PHASE12L_N256_FULL_FINAL_TIME_1_0_FEASIBILITY_AUDIT.csv")
PAIRWISE_OUT_CSV = Path("PHASE12L_N256_FULL_FINAL_TIME_1_0_FEASIBILITY_PAIRWISE.csv")
...
print("Purpose: run full final-time-1.0 N256 feasibility across fd_centered, pseudo_spectral, and arakawa.")
...
reference_solver = SpectralSolver(
    ...
    ny=N,
    Re=RE,
    run_path=Path("experiments") / "reference" / "phase12L_N256_reference_grid",
    dt=DT,
    steps=STEPS,
)
...
for method in METHODS:
    solvers[method] = SelectableAdvectionSolver(
        ...
        ny=N,
        Re=RE,
        run_path=Path("experiments") / "selectable_diagnostics" / f"phase12L_N256_{method}",
        dt=DT,
        steps=STEPS,
...
dt_ok = DT == 0.001
steps_ok = STEPS == 1000
final_time_ok = abs(FINAL_TIME - 1.0) <= 1e-15
log_every_ok = LOG_EVERY == 100
...
print(f"steps == 1000: {pass_fail(steps_ok)}")
print(f"final time == 1.0: {pass_fail(final_time_ok)}")
print(f"log_every == 100: {pass_fail(log_every_ok)}")
...
for method in METHODS:
    ...
    print(f"elapsed_seconds: {row['elapsed_seconds']:.12e}")
...
print("\n=== OVERALL RESULT ===")
print(f"Phase 12L N256 full final-time-1.0 feasibility audit: {pass_fail(overall_pass)}")
...
print("\n=== SCIENTIFIC BOUNDARY ===")
print("This is an N256 full final-time-1.0 feasibility audit only.")
...
print("Phase 12L audit complete.")