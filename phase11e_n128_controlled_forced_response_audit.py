...
OUT_AUDIT_CSV = Path("PHASE11E_N128_CONTROLLED_FORCED_RESPONSE_AUDIT.csv")
OUT_TIME_CSV = Path("PHASE11E_N128_CONTROLLED_FORCED_RESPONSE_TIME_HISTORY.csv")
OUT_PAIRWISE_CSV = Path("PHASE11E_N128_CONTROLLED_FORCED_RESPONSE_PAIRWISE_SUMMARY.csv")
...
def audit_invalid_method():
    try:
        SelectableAdvectionSolver(
            ...
            ny=N,
            Re=RE,
            run_path=Path("experiments") / "method_diagnostics" / "phase11E_invalid",
            dt=DT,
            steps=1,
...
def make_solver(method):
    return SelectableAdvectionSolver(
        ...
        ny=N,
        Re=RE,
        run_path=Path("experiments") / "method_diagnostics" / f"phase11E_{method}",
        dt=DT,
        steps=STEPS,
...
def forcing_checks_across_methods():
    solvers = {method: make_solver(method) for method in METHODS}
...
def run_method_response(method, initial_field):
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % LOG_EVERY == 0 or step == STEPS:
            ...
            print(
                ...
                f"time={step * DT:.3f}, "
                f"rms={row['rms_vorticity']:.12e}, "
                f"E={row['kinetic_energy']:.12e}, "
                f"Z={row['enstrophy']:.12e}, "
    ...
    relative_energy_change = (final_energy - initial_energy) / max(abs(initial_energy), 1e-300)
    relative_enstrophy_change = (final_enstrophy - initial_enstrophy) / max(abs(initial_enstrophy), 1e-300)
    ...
    final_rms_nonexplosive = final_rms_ratio < 2.0
    final_energy_nonexplosive = final_energy_ratio < 4.0
    final_enstrophy_nonexplosive = final_enstrophy_ratio < 4.0
    ...
    summary = {
        ...
        "forcing_rms": rms(force),
        "forcing_l2": l2_norm(force),
        "forcing_max_abs": max_abs(force),
...
def pairwise_final_summary(method_a, method_b, final_fields, solvers):
    ...
    no_large_pairwise_disagreement = rel_error < 0.05
...
print("Purpose: N=128 controlled forced response across fd_centered, pseudo_spectral, and arakawa.")
...
forcing_global = forcing_checks_across_methods()
...
for method in METHODS:
    print(f"\n=== METHOD FORCED RESPONSE: {method} ===")

    summary, time_rows, final_field, solver = run_method_response(method, initial_field)
    ...
    print(f"forcing_nonzero: {summary['forcing_nonzero']}")
    print(f"forcing_rms: {summary['forcing_rms']:.12e}")
    ...
    print(f"relative_energy_change: {summary['relative_energy_change']:.12e}")
    print(f"final_energy_ratio: {summary['final_energy_ratio']:.12e}")
    print(f"relative_enstrophy_change: {summary['relative_enstrophy_change']:.12e}")
...
print(f"Method forced-response checks pass: {pass_fail(method_pass)}")
print(f"Final pairwise checks pass: {pass_fail(pairwise_pass)}")
print(f"Phase 11E N128 controlled forced-response audit: {pass_fail(overall_pass)}")

print("\n=== OUTPUT WRITTEN ===")
print(f"Wrote: {OUT_AUDIT_CSV}")
print(f"Wrote: {OUT_TIME_CSV}")
print(f"Wrote: {OUT_PAIRWISE_CSV}")
print("Phase 11E N128 controlled forced-response audit complete.")