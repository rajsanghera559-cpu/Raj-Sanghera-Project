"""
Controlled remediated Stage D1 separately advanced advection-form trajectory pilot.

Usage:
    python -B run_stage_d_separate_trajectory_pilot_remediated.py inspect
    python -B run_stage_d_separate_trajectory_pilot_remediated.py run
...
The separately authorized guarded run path advances exactly seven independently owned vorticity
...
This remediated pilot does not classify trajectory agreement, rank methods, fit spectral
...
RUNNER_NAME = "run_stage_d_separate_trajectory_pilot_remediated.py"
...
AUTHORIZED_DESIGN_COMMIT_PREFIX = "beea25c"

DESIGN_PATH = Path(
    "STAGE_D1_PILOT_FAILURE_REMEDIATION_DESIGN.md"
)
EXPECTED_DESIGN_SHA256 = (
    "AE37B9226F6C0A58891A7C5167F9F7FCB540C616827D41C0E593655C6D2BD81B"
)

ORIGINAL_STAGE_D_DESIGN_PATH = Path(
    "STAGE_D_SEPARATELY_ADVANCED_ADVECTION_FORM_TRAJECTORY_COMPARISON_DESIGN.md"
)
EXPECTED_ORIGINAL_STAGE_D_DESIGN_SHA256 = (
    "491B1118B40523CF4EC340E40C38CAC462C8985377809D09484571884FC445BF"
)

FAILURE_DIAGNOSIS_PATH = Path("STAGE_D1_PILOT_FAILURE_DIAGNOSIS.md")
EXPECTED_FAILURE_DIAGNOSIS_SHA256 = (
    "526E4243C40D850F3CA5F158AB04F783283C299754C6CA759FAB5EA7536743C0"
)

FAILED_RUNNER_PATH = Path("run_stage_d_separate_trajectory_pilot.py")
EXPECTED_FAILED_RUNNER_SHA256 = (
    "BF23D8C755810988821B1A36CBD3023EB839B9EF8BE0EF5F31B808EF239E14B7"
)
...
FAILED_STAGE_D1_EVIDENCE_DIRECTORY = (
    Path("experiments")
    / "advection_form_trajectory_pilot"
    / "stage_d_separate_trajectory_pilot_20260721T041525Z_a5397a6"
)
FAILED_STAGE_D1_EVIDENCE_HASHES = {
    "file_inventory.csv":
        "8685631746E8F7F7B6E97C95775E6B6A7BF8155A14364ED90458B563FE0F7EB9",
    "run_metadata.json":
        "D531E482E4CCC1E08443C5C897BC4FFA8BCD3219303820A87BB55CD0755384AA",
    "trajectory_pilot_diagnostics.csv":
        "32E44544395EA31C6CD7D5D1FC1F01606A81AAC0764126D394A91DC102F831A8",
    "trajectory_pilot_integrity_per_step.csv":
        "30B702EA4BA313C516FC9CE4AC1900A00223A17998EECA363F698B95773A22DA",
    "trajectory_pilot_pairwise_divergence.csv":
        "83492FD26F5833BDAF918B56CBA8A8146E8E52F111BA6E9E9A5DF5B99FCC910A",
    "trajectory_pilot_sentinel_crosscheck.csv":
        "CEF07116A5A8CED495B870A486C4AAC73B1188F2D34F803D7D4148624D9135F8",
}
...
OUTPUT_ROOT = Path("experiments") / "advection_form_trajectory_pilot_remediated"
RUN_PREFIX = "stage_d_separate_trajectory_pilot_remediated_"
...
PREDICTED_OUTPUT_FILE_BYTES = {
    ...
    "trajectory_pilot_integrity_per_step.csv": 35_000_000,
    "trajectory_pilot_sentinel_crosscheck.csv": 250_000,
    "trajectory_pilot_summary.json": 500_000,
    "STAGE_D_REMEDIATED_SEPARATE_TRAJECTORY_PILOT_REPORT.md": 250_000,
...
def verify_source_identities(repo: Path) -> dict[str, str]:
    hashes = {
        "stage_d_remediation_design": verify_file_hash(
            repo, DESIGN_PATH, EXPECTED_DESIGN_SHA256
        ),
        "stage_d_original_design": verify_file_hash(
            repo,
            ORIGINAL_STAGE_D_DESIGN_PATH,
            EXPECTED_ORIGINAL_STAGE_D_DESIGN_SHA256,
        ),
        "stage_d_failure_diagnosis": verify_file_hash(
            repo, FAILURE_DIAGNOSIS_PATH, EXPECTED_FAILURE_DIAGNOSIS_SHA256
        ),
        "stage_d_failed_runner": verify_file_hash(
            repo, FAILED_RUNNER_PATH, EXPECTED_FAILED_RUNNER_SHA256
        ),
    ...
    for name, value in verify_evidence_inventory(
        repo / FAILED_STAGE_D1_EVIDENCE_DIRECTORY,
        FAILED_STAGE_D1_EVIDENCE_HASHES,
    ).items():
        hashes[f"failed_stage_d1/{name}"] = value
    return hashes
...
def verify_inspection_repository_state(repo: Path, runner: Path) -> str:
    ...
    if status_lines != expected:
        raise RuntimeError(
            f"inspection requires exactly one untracked remediated pilot runner; observed={status_lines!r}"
        )
...
def verify_run_preflight(repo: Path, runner: Path) -> tuple[str, dict[str, str]]:
    ...
    if OUTPUT_ROOT.exists():
        ...
        if prior:
            raise RuntimeError(f"prior remediated Stage D pilot output exists: {prior}")
    probe = repo / OUTPUT_ROOT / (RUN_PREFIX + "ignore_probe")
    if not path_is_git_ignored(repo, probe):
        raise RuntimeError(
            f"remediated pilot output path is not Git-ignored: {probe.relative_to(repo)}"
        )
    oversized = {
        name: size
    ...
    *,
    ...
    update_finite = all(
        ...
        for value in (
            ...
            maximum_imaginary_ratio, mask_loss_physical, mask_loss_spectral,
            mask_enstrophy_change_rate, observed_mask_change_rate,
            filter_bookkeeping_residual, normalized_filter_bookkeeping,
        )
    )
    subgate_passes = {
        "post_filter_update_finite": bool(update_finite),
        "unfiltered_ledger_closure": bool(
            update_finite
            and normalized_unfiltered <= UNFILTERED_LEDGER_CLOSURE_LIMIT
        ),
        "filtered_ledger_closure": bool(
            update_finite
            and normalized_filtered <= FILTERED_LEDGER_CLOSURE_LIMIT
        ),
        "mask_parseval_crosscheck": bool(
            update_finite
            and normalized_mask_crosscheck <= MASK_CROSSCHECK_LIMIT
        ),
        "real_compatible_imaginary_ratio": bool(
            update_finite and maximum_imaginary_ratio <= IMAGINARY_RATIO_LIMIT
        ),
    }
    failed_subgates = tuple(
        name for name, passed in subgate_passes.items() if not passed
    )
    integrity_pass = not failed_subgates
    ...
    ledger_scalars = (
        ...
        mask_enstrophy_change_rate, observed_unfiltered_rate,
    ...
    return {
        ...
        "mask_enstrophy_change_rate": mask_enstrophy_change_rate,
        "observed_mask_change_rate": observed_mask_change_rate,
        ...
        "filter_bookkeeping_residual": filter_bookkeeping_residual,
...
def failure_snapshot_from_preview(
    preview: Mapping[str, object],
) -> dict[str, object]:
    failed_subgates = tuple(str(name) for name in preview["failed_subgates"])
    subgate_passes = dict(preview["subgate_passes"])
    return {
        "trajectory_id": str(preview["trajectory_id"]),
        "loop_index": int(preview["loop_index"]),
        "completed_steps_before_failure": int(preview["loop_index"]),
        "failed_stage": "post_filter_integrity",
        "primary_failed_subgate": failed_subgates[0] if failed_subgates else None,
        "all_failed_subgates": list(failed_subgates),
        "subgate_passes": subgate_passes,
        "subgate_values": {
            "update_finite": preview["update_finite"],
            "normalized_unfiltered_closure": preview[
                "normalized_unfiltered_closure"
            ],
            "normalized_filtered_closure": preview[
                "normalized_filtered_closure"
            ],
            "normalized_mask_crosscheck": preview["normalized_mask_crosscheck"],
            "maximum_imaginary_ratio": preview["maximum_imaginary_ratio"],
        },
        "subgate_limits": {
            "post_filter_update_finite": True,
            "unfiltered_ledger_closure": UNFILTERED_LEDGER_CLOSURE_LIMIT,
            "filtered_ledger_closure": FILTERED_LEDGER_CLOSURE_LIMIT,
            "mask_parseval_crosscheck": MASK_CROSSCHECK_LIMIT,
            "real_compatible_imaginary_ratio": IMAGINARY_RATIO_LIMIT,
        },
        "z_current": preview["z_current"],
        "z_stage": preview["z_stage"],
        "z_unfiltered": preview["z_unfiltered"],
        "z_filtered": preview["z_filtered"],
        "mask_loss_physical": preview["mask_loss_physical"],
        "mask_loss_spectral": preview["mask_loss_spectral"],
        "mask_enstrophy_change_rate": preview["mask_enstrophy_change_rate"],
        "observed_mask_change_rate": preview["observed_mask_change_rate"],
        "filter_bookkeeping_residual": preview["filter_bookkeeping_residual"],
        "normalized_filter_bookkeeping": preview[
            "normalized_filter_bookkeeping"
        ],
        "normalized_mask_crosscheck": preview["normalized_mask_crosscheck"],
        "normalized_unfiltered_closure": preview[
        ...
        "maximum_imaginary_ratio": preview["maximum_imaginary_ratio"],
        "state_hashes": {
            "current": sha256_array(preview["current"]),
            "stage": sha256_array(preview["stage"]),
            "accepted": sha256_array(preview["accepted"]),
        },
    }


def build_integrity_row(
    preview: Mapping[str, object],
    *,
    state_mutation_count: int,
    state_alias_count_value: int,
    integrity_pass: bool,
) -> dict[str, object]:
    subgate_passes = dict(preview["subgate_passes"])
    return {
        ...
        "physical_time": preview["physical_time"],
        ...
        "state_alias_count": state_alias_count_value,
        "update_finite": preview["update_finite"],
        "update_finite_pass": subgate_passes["post_filter_update_finite"],
        "unfiltered_closure_pass": subgate_passes[
            "unfiltered_ledger_closure"
        ],
        "filtered_closure_pass": subgate_passes["filtered_ledger_closure"],
        "mask_crosscheck_pass": subgate_passes["mask_parseval_crosscheck"],
        "imaginary_ratio_pass": subgate_passes[
            "real_compatible_imaginary_ratio"
        ],
        "failed_subgates": ";".join(str(name) for name in preview["failed_subgates"]),
        "integrity_pass": integrity_pass,
    }
...
def render_report(
    ...
    return "\n".join(
        (
            "# Stage D Remediated Separate-Trajectory Pilot Report",
            "",
            "## Control",
            "",
            f"- Run ID: `{run_id}`",
            ...
            f"- Maximum normalized mask cross-check: `{maximums['mask_crosscheck']:.17g}`",
            f"- Maximum normalized filter bookkeeping: `{maximums['filter_bookkeeping']:.17g}`",
...
def inspect_ast(source: str) -> dict[str, object]:
    ...
    required_fragments = (
        ...
        "mask_enstrophy_change_rate = -mask_loss_physical / solver.dt",
        "observed_mask_change_rate = (z_filtered - z_unfiltered) / solver.dt",
        "stage=\"post_filter_integrity\"",
        "failure_snapshot_from_preview(preview)",
    )
    ...
    forbidden_fragments = (
        "mask_rate_" + "physical = (z_filtered - z_unfiltered)",
        "mask_rate_" + "spectral = (",
        "stage_" + "d2",
        "STAGE_" + "D2",
    )
    observed_forbidden_fragments = [
        fragment for fragment in forbidden_fragments if fragment in source
    ]
    if observed_forbidden_fragments:
        raise RuntimeError(
            "forbidden remediated source fragments present: "
            f"{observed_forbidden_fragments}"
        )
    return {
...
def inspect_runner(repo: Path) -> int:
    ...
    if OUTPUT_ROOT != Path("experiments") / "advection_form_trajectory_pilot_remediated":
        fail("remediated output root differs from frozen design")
    if RUN_PREFIX != "stage_d_separate_trajectory_pilot_remediated_":
        fail("remediated run prefix differs from frozen design")
    ...
    if not required_integrity_fields.issubset(INTEGRITY_FIELDNAMES):
        fail("remediated integrity schema is incomplete")
    if any(size >= MAX_OUTPUT_FILE_BYTES for size in PREDICTED_OUTPUT_FILE_BYTES.values()):
        fail("predicted remediated output reaches the frozen 40 MB limit")
    ...
    print()
    print("=" * 72)
    print("STAGE D REMEDIATED SEPARATE-TRAJECTORY PILOT RUNNER INSPECTION: PASS")
    print("=" * 72)
    ...
    print("Remediation design SHA256:", source_hashes["stage_d_remediation_design"])
    print("Failure diagnosis SHA256:", source_hashes["stage_d_failure_diagnosis"])
    print("Failed runner SHA256:", source_hashes["stage_d_failed_runner"])
    ...
    print("Stage B physical/Fourier mask definition: PRESENT")
    print("Whole-field filter bookkeeping separated: PRESENT")
    print("Independent post-filter sub-gates: PRESENT")
    print("Failure snapshot contract: PRESENT")
    ...
    print("Stage D1R numerical execution authorized by inspection: NO")
    print("Stage D2 output or execution path: ABSENT")
...
def execute_pilot(repo: Path) -> int:
    ...
    report_path = (
        run_directory / "STAGE_D_REMEDIATED_SEPARATE_TRAJECTORY_PILOT_REPORT.md"
    )
    inventory_path = run_directory / "file_inventory.csv"

    metadata: dict[str, object] = {
        "schema_id": "STAGE_D_REMEDIATED_SEPARATE_TRAJECTORY_PILOT_METADATA_V1",
        "run_id": run_id,
        "status": "running",
    ...
    try:
        ...
        for loop_index in range(PILOT_UPDATES):
            ...
            for trajectory_id in TRAJECTORY_IDS:
                ...
                if not bool(preview["integrity_pass"]):
                    ...
                    integrity_writer.write(
                        build_integrity_row(
                            ...
                            state_alias_count_value=alias_count,
                            integrity_pass=False,
                        )
                    )
                    integrity_writer.flush()
                    failed_integrity_rows += 1
                    failure_snapshot = failure_snapshot_from_preview(preview)
                    failure_snapshot["failed_integrity_row_written"] = True
                    metadata["failure_snapshot"] = failure_snapshot
                    metadata["progress"] = {
                        "last_completed_loop_index": last_completed_loop,
                        **counts,
                        "failed_integrity_rows": failed_integrity_rows,
                    }
                    atomic_write_json(metadata_path, metadata)
                    failed_subgates = tuple(preview["failed_subgates"])
                    primary_gate = str(failed_subgates[0])
                    raise IntegrityFailure(
                        primary_gate,
                        (
                            f"post-filter integrity failed for {trajectory_id} "
                            f"at loop {loop_index}: {failed_subgates}"
                        ),
                        trajectory_id=trajectory_id,
                        loop_index=loop_index,
                        stage="post_filter_integrity",
                        details=failure_snapshot,
                    )
            ...
            for trajectory_id in TRAJECTORY_IDS:
                ...
                if not row_pass:
                    ...
                    failure_snapshot.update(
                        {
                            "failed_stage": "accepted_state_integrity",
                            "primary_failed_subgate": "per_step_integrity",
                            "all_failed_subgates": ["per_step_integrity"],
                            "state_mutation_count": state_mutation_count,
                            "state_alias_count": alias_count,
                            "failed_integrity_row_written": True,
                        }
                    )
                    metadata["failure_snapshot"] = failure_snapshot
                    metadata["progress"] = {
                    ...
                    raise IntegrityFailure(
                        ...
                        trajectory_id=trajectory_id,
                        loop_index=loop_index,
                        stage="accepted_state_integrity",
                        details=failure_snapshot,
                    )
                integrity_writer.write(
                    build_integrity_row(
                ...
                maximums["mask_crosscheck"] = max(
                    maximums["mask_crosscheck"],
                    float(preview["normalized_mask_crosscheck"]),
                )
                maximums["filter_bookkeeping"] = max(
                    maximums["filter_bookkeeping"],
        ...
        completed = utc_text()
        summary = {
            "schema_id": "STAGE_D_REMEDIATED_SEPARATE_TRAJECTORY_PILOT_SUMMARY_V1",
            "run_id": run_id,
            "status": "pass",
        ...
        print()
        print("=" * 72)
        print("STAGE D REMEDIATED SEPARATE-TRAJECTORY PILOT: PASS")
        print("=" * 72)
        print("Pilot updates per trajectory: 3060")
        ...
        if isinstance(error, IntegrityFailure):
            ...
            if error.details:
                metadata["failure_snapshot"] = error.details
        else:
        ...
        if "failure_snapshot" not in metadata:
            metadata["failure_snapshot"] = {
                "trajectory_id": failed_trajectory,
                "loop_index": failed_loop,
                "completed_steps_before_failure": max(last_completed_loop + 1, 0),
                "failed_stage": failed_stage,
                "primary_failed_subgate": failed_gate,
                "all_failed_subgates": [failed_gate],
                "failed_integrity_row_written": False,
                "available_values": "failure occurred before a complete preview snapshot",
            }
        metadata["progress"] = {
            "last_completed_loop_index": last_completed_loop,
            **counts,
        ...
        print()
        print("STAGE D REMEDIATED SEPARATE-TRAJECTORY PILOT: FAILED")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect or execute the controlled remediated Stage D "
